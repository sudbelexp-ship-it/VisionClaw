...
final class SimultaneousInterpreter: ObservableObject {
    ...
    func start(source: TranslatorLanguage) async {
        ...
        do {
            ...
            // Никакого requiresPhoneMic: переводчик подчиняется той же настройке «слушать очками,
            // пока молчим», что и всё остальное. Это делает его поочерёдным, а не действительно
            // одновременным: пока в ухе звучит перевод — микрофон выключен (полное качество звука,
            // как в наушниках), собеседник в этот момент не расслышан; как только перевод
            // заканчивается — включается микрофон очков и слушает дальше. Тот же ритм, что у
            // обычного переводчика-гаджета: сказал — послушал перевод — говори снова. Настройка
            // выключена по умолчанию, так что для тех, кто её не трогал, ничего не меняется —
            // переводчик остаётся на телефоне, как раньше.
